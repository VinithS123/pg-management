...
async function handleInlineCustomerEdit(e) {
    ...
    const confirmed = await showConfirmationToast('Are you sure you want to save this change?', 'Confirm Update');
    if (!confirmed) {
    ...
    try {
        ...
        showToast('Update completed successfully.', 'Success', 'success');
    } catch (err) {
        target.value = previousValue;
    }
}
...
function handleApiError(error) {
    if (error.status === 401 || error.status === 403) {
        logout();
        showToast('Session expired. Please log in again.', 'Authentication Error', 'error');
    } else {
        showToast('An error occurred. Please try again later.', 'Process Failed', 'error');
    }
}
    ...
    try {
        ...
        if (response.ok) {
            ...
            if (token === "User Verification Unsuccessful" || token.includes("Unsuccessful") || token === "Fail") {
                showToast('Invalid credentials. Please verify your details.', 'Login Failed', 'error');
                return;
            }
            ...
            showToast('Invalid credentials. Please try again.', 'Login Failed', 'error');
        }
    } catch (e) {
        showToast('Unable to connect to the server.', 'Connection Error', 'error');
    }
}
...
function getRoomAvailableBeds(room) {
    ...
    return Math.max(totalBeds - getRoomOccupiedBeds(room), 0);
}
...
async function loadRooms() {
    try {
        setRoomSyncStatus('Loading room status...');
        ...
        roomsList = [...reservedRooms, ...unreservedRooms].map(room => ({
            ...
            availableBeds: getRoomAvailableBeds({ ...room, occupiedBeds: occupancyMap.get(Number(room.roomNo)) || 0 }),
            calculatedStatus: getDerivedRoomStatus({ ...room, occupiedBeds: occupancyMap.get(Number(room.roomNo)) || 0 })
        }));
        ...
        if (reservedResult.status === 'rejected' || unreservedResult.status === 'rejected') {
            setRoomSyncStatus('Data load incomplete.', 'warning');
        } else {
            setRoomSyncStatus('Room status loaded.', 'success');
        }
    } catch (e) {
        roomAvailabilityLoaded = false;
        roomOccupancyLoaded = false;
        setRoomSyncStatus('Unable to load rooms.', 'danger');
    }
}
...
function renderRoomsTable() {
    ...
    const filteredRooms = roomsList
        ...
        .filter(room => !searchTerm || String(room.roomNo).includes(searchTerm))
        .sort((a, b) => Number(a.roomNo) - Number(b.roomNo));

    tbody.innerHTML = '';

    filteredRooms.forEach(room => {
        const calculatedStatus = room.calculatedStatus || room.roomStatus;
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td>${room.roomNo ?? ''}</td>
            <td>${getRoomOccupiedBeds(room)} / ${room.noOfBed ?? ''}</td>
            <td>${getRoomAvailableBeds(room)}</td>
            <td><span class="status-badge ${calculatedStatus === 'RESERVED' ? 'status-pending' : 'status-active'}">${calculatedStatus === 'RESERVED' ? 'Reserved' : 'Unreserved'}</span></td>
        `;
...
function updateCustomerRoomAvailability(roomNo) {
    ...
    if (!matchingRoom) {
        customerRoomIsAllocated = false;
        hint.textContent = roomAvailabilityLoaded ? 'Room not found.' : 'Status will be checked on save.';
    ...
    hint.textContent = 'Room fully occupied';
...
async function saveRoom(roomData) {
    try {
        ...
        showToast('Room details saved.', 'Success', 'success');
...
async function deleteCustomer(id) {
    if(confirm('Are you sure you want to proceed with deletion?')) {
        try {
            ...
            showToast('Customer record removed.', 'Success', 'success');
            loadCustomers();
        } catch (e) { }
    }
}
        ...
        accessoriesList.innerHTML = accessories.length
            ? accessories.map(acc => `
                <div class="detail-item detail-full">
                    <span class="detail-label">${acc.accessoriesName || 'Item'}</span>
                    <strong class="detail-value">${acc.accessoriesPrice ?? '-'}</strong>
                </div>
            `).join('')
            : '<div class="empty-state">No rented items.</div>';
    }
...
async function saveCustomer(formData) {
    ...
    try {
        ...
        if (customerRoomIsAllocated) {
            showToast('Room constraint validation failed.', 'Action Blocked', 'error');
            return;
        }
...
function setupEventListeners() {
    ...
    document.getElementById('message-all-btn').addEventListener('click', async (e) => {
        const btn = e.target;
        ...
        if (selectedCustomers.length === 0) {
            showToast('Selection required to perform this action.', 'Validation', 'warning');
            return;
        }
